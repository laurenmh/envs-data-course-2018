...
library(knitr)
mydat <- read.csv("PollutionData_Exercise.csv")
ggplot(mydat, aes(x=PM10, y=Mortality, color = season)) + geom_point(size = 6) + geom_smooth(method = "lm", se = "F") + theme_classic() + labs(x="PM10 concentration", y="Daily mortality (all causes)", color="Season") + theme(text=element_text(size = 24))

ggplot(mydat, aes(x=PM10, y=Mortality)) + geom_point(size = 6) + geom_smooth(method = "lm", se = "F") + theme_classic() + labs(x="PM10 concentration", y="Daily mortality (all causes)", color="Season") + theme(text=element_text(size = 24))
```
